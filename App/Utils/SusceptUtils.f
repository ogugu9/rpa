...
c#########################################################
c#### productWaveFunction: productions of wave functions
c#########################################################

      subroutine productWaveFunction(iq)

      use common, only : Nband, Nkpt, Nksize, Nkpath, Nkmesh, Nqx,
     & Zpsiall, Zwfprod, Nqx
      implicit none
      ...
      do ik = 1, Nkpt
            do iorb1 = 1, nb ; do iorb2 = 1, nb
               do n = 1, nb ; do is = 1, 2
                  ...
                  zval
     &                 =        Zpsiall(ik,iq,iorb2,n,is)
     &                 * DCONJG(Zpsiall(ik,iq,iorb1,n,is))
                  if (ABS(DBLE(zval)) < 1.0d-14)
     &                 zval = DCMPLX(0.0d0,DIMAG(zval))
                  if (ABS(DIMAG(zval)) < 1.0d-14)
     &                 zval = DCMPLX(DBLE(zval),0.0d0)
                  Zwfprod(ik,iq,iorb1,iorb2,n,is) = zval
c                  if (iq .eq. 0) then
c                  write(*,*) ik,Zwfprod(ik,iq,iorb1,iorb2,n,is)
c                  endif
               end do ; end do
         end do ; end do
      end do ;
