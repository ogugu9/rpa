...
c######################################################################
      ...
      subroutine estimateChemicalPotential(ddmu)
      implicit none

      real*8, intent(inout) :: ddmu
      real*8 :: dx1, dx2, dxacc, d1, d2
      real*8, external :: Drtbis, diffElectronDensity

      if (diffElectronDensity(ddmu) == 0.0d0) then
         return
      else
         dx1 = ddmu - 1.0d0
         dx2 = ddmu + 1.0d0
         ...
         d1 = diffElectronDensity(dx1)
         d2 = diffElectronDensity(dx2)
         ...
         ddmu = Drtbis(diffElectronDensity,dx1,dx2,Dxacc)
      end if
...
c **********************************
      double precision function diffElectronDensity(ddmu)

      use common, only: Dne, Nband
      implicit none

      real*8, intent(in) :: ddmu
      real*8, external :: ntotal

      ntotal = Dntotal(ddmu) * DBLE(Nband) - Dne
      return
      end
c **********************************
...
c******* Fermi function  (Degn:energy, DkT:Temperature)
      ...
      real*8, intent(in) :: Dffn, Degn, DkT
      real*8 :: De, ddd
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      double precision FUNCTION Drtbis(diffeElectronDensity,x1,x2,xacc)
      INTEGER JMAX
      REAL*8 x1,x2,xacc,diffElectronDensity
      EXTERNAL diffElectronDensity
      PARAMETER (JMAX=40)
      INTEGER j
      REAL*8 dx,f,fmid,xmid

      fmid=diffElectronDensity(x2)
      f=diffElectronDensity(x1)
